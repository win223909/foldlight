...
BUILD_DIR="${FOLDLIGHT_BUILD_DIR:-$ROOT_DIR/build/MacApp}"
APP_DIR="$BUILD_DIR/Foldlight.app"
SDK_PATH="$(xcrun --sdk macosx --show-sdk-path)"
mkdir -p "$APP_DIR/Contents/MacOS" "$APP_DIR/Contents/Resources"
for arch in arm64 x86_64; do
  xcrun swiftc -swift-version 5 -O -sdk "$SDK_PATH" -target "$arch-apple-macos13.0" \
    "$ROOT_DIR"/macos/Sources/*.swift -o "$BUILD_DIR/Foldlight-$arch"
done
lipo -create "$BUILD_DIR/Foldlight-arm64" "$BUILD_DIR/Foldlight-x86_64" -output "$APP_DIR/Contents/MacOS/Foldlight"
