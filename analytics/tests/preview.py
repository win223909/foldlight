...
with tempfile.TemporaryDirectory() as tmp:
    ...
    log.write_text(''.join(json.dumps(dict(at=time.time()-i*2300,ip='203.0.113.'+str(i%12+1),method='GET',path='/' if i%7 else '/downloads/Foldlight-0.2.3-macOS.dmg',status=200,ua=ua[i%2],country='CN',referrer='example.com',bytes=100))+'\n' for i in range(100)))
    store.ingest(log)
