...
final class LocalHingeInput implements AutoCloseable {
    ...
    private final Shizuku.OnRequestPermissionResultListener permission=(code,result)->{
        if(code==26){if(result==PackageManager.PERMISSION_GRANTED)connect();else state(false,false,"未获授权 · 可在 Shizuku 中允许折光 Fold");}
    };
    ...
    LocalHingeInput(Context context,Listener listener){
        ...
        args=new Shizuku.UserServiceArgs(new ComponentName(context,LocalHingeService.class)).daemon(false).tag("foldlight-local-hinge").processNameSuffix("hinge").version(BuildConfig.VERSION_CODE);
    ...
    boolean request(){
        ...
        try{
            ...
            else if(Shizuku.shouldShowRequestPermissionRationale()){state(false,false,"请在 Shizuku → 已授权应用中允许折光 Fold");return false;}
