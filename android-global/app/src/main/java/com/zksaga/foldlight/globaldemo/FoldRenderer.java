...
public final class FoldRenderer implements GLSurfaceView.Renderer {
    ...
    @Override public synchronized void onSurfaceCreated(GL10 gl,EGLConfig config) {
        try {
            uniformLocations.clear();program=GLES30.glCreateProgram();
    ...
    private int uniform(String name) { Integer cached=uniformLocations.get(name);if(cached!=null)return cached;int location=GLES30.glGetUniformLocation(program,name);uniformLocations.put(name,location);return location; }
